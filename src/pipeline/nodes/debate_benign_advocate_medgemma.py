...
# Check if debate should use MedGemma (selective routing)
USE_MEDGEMMA = os.getenv("DEBATE_USE_MEDGEMMA", "false").lower() == "true"
if USE_MEDGEMMA:
    from src.utils.medgemma_client import call_medgemma_json
    logger.info("Benign advocate will use MedGemma for debate")
...
def _query_clinvar_benign(hgvs_p: Optional[str], hgvs_c: Optional[str], gene: str, genome_build: str = "GRCh38") -> list[str]:
    """
...
def debate_benign_advocate_node(state: VariantState) -> dict:
    ...
    clinvar_chunks   = _query_clinvar_benign(hgvs_p, hgvs_c, gene, genome_build)
    ...
    # Use MedGemma if flag is set, otherwise use Bedrock gpt-oss-20b
    if USE_MEDGEMMA:
        raw_result = call_medgemma_json(
            system_prompt,
            user_prompt,
            temperature=0.2,
            max_tokens=4096,
