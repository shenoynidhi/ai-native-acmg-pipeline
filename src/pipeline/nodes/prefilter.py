...
def prefilter_node(state: VariantState) -> dict:
    ...
    Prefers cleaned_vcf (with alternate contigs removed) if available,
    otherwise falls back to original proband_vcf_path.
    """
    session_id = state["session_id"]
    warnings   = list(state.get("warnings", []))
    cfg        = PipelineConfig()   # uses defaults; API layer can override

    # Prefer cleaned VCF (alternate contigs removed), fallback to original
    ...
    has_info_af = "INFO=<ID=AF," in header  # gnomAD allele frequency
    ...
    # MAF filter is NOT applied here (pre-VEP)
    # Reason: Input VCF often lacks population frequency (INFO/AF is sample AF, not gnomAD)
    # MAF filtering happens post-VEP in post_process node using gnomAD data from VEP CSQ
    if cfg.maf_threshold > 0:
        logger.info(
            f"[{session_id}] MAF filtering will be applied post-VEP (threshold: {cfg.maf_threshold*100}%) "
            f"using gnomAD population frequencies from VEP annotation"
        )
    else:
        logger.info(f"[{session_id}] MAF filtering disabled (threshold=0) — all variants will be processed")
    ...
    logger.debug(f"[{session_id}] Prefilter returning: filtered_vcf={return_dict['filtered_vcf']}")
    return return_dict
