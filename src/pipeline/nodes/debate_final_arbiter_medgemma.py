...
if USE_MEDGEMMA:
    ...
    logger.info("Final arbiter will use MedGemma for debate")
...
def debate_final_arbiter_node(state: VariantState) -> dict:
    ...
    if USE_MEDGEMMA:
        raw_result = call_medgemma_json(
            ...
            user_prompt,
            temperature=0.2,
            max_tokens=8000,  # MedGemma max limit (arbiter needs lots of tokens!)
        )
    else:
        raw_result = call_llm_json(
            system_prompt,
            user_prompt,
            max_tokens=6000,  # Bedrock can handle more
