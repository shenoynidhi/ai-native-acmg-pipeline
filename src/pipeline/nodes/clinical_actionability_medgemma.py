...
# Check if clinical actionability should use MedGemma (selective routing)
USE_MEDGEMMA = os.getenv("ACTIONABILITY_USE_MEDGEMMA", "false").lower() == "true"
if USE_MEDGEMMA:
    from src.utils.medgemma_client import call_medgemma_json
    logger.info("Clinical actionability will use MedGemma")
...
def clinical_actionability_node(state: VariantState) -> dict:
    ...
    # Call LLM (MedGemma or Bedrock depending on flag)
    try:
        if USE_MEDGEMMA:
            result = call_medgemma_json(
                system_prompt,
                user_prompt,
                temperature=0.2,
                max_tokens=4096,
