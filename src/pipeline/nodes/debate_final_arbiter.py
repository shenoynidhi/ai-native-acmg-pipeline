...
3. If both meaningful pathogenic AND benign evidence exist simultaneously:
   classify as VUS — do NOT resolve the conflict by ignoring one side.
...
def _validate_arbiter_output(raw: dict, state: VariantState, variant_id: str) -> dict:
    ...
    for crit in unevaluated:
        flag = f"{crit}_not_evaluated_no_clinical_history"
