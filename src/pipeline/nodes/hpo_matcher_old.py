...
# (gene_symbol, orpha_code) → inheritance string (e.g. ("BRCA2", "ORPHA:227535") → "AD")
_GENE_DISEASE_TO_INHERITANCE: Dict[tuple, str] = {}

# gene_symbol → list of (orpha_code, inheritance) — for fallback when no HPO match
_GENE_TO_DISEASE_INHERITANCE: Dict[str, List[tuple]] = defaultdict(list)
_INHERITANCE_LOADED = False

# OMIM morbidmap fallback
# gene symbol (upper) → set of inheritance short codes
_OMIM_GENE_TO_INHERITANCE: Dict[str, Set[str]] = defaultdict(set)
_OMIM_LOADED = False

# Normalise Orphanet full strings → short codes zygosity_filter understands
_INH_NORMALISE = {
    ...
    "mitochondrial":           "MITO",
    "y-linked":                "YL",
    # these are not actionable for zygosity — skip them
    "multigenic/multifactorial": None,
    "not applicable":           None,
    "unknown":                  None,
    "":                         None,
}

# Association types that mean this gene *causes* the disease
# Skip susceptibility/modifier roles — they have unreliable inheritance
_CAUSAL_ASSN_TYPES = {
...
def _load_orphanet_inheritance_tsv() -> None:
    """
    Load Orphanet gene-disease-inheritance mappings from TSV.
    Filters by causal association types and normalises to short codes.
    Stores per-gene-disease pairs for precise inheritance lookup.
    Columns: geneSymbol, inheritance, assnType, orphaCode
    """
    ...
    with open(tsv_path, encoding="utf-8") as fh:
        ...
        for row in reader:
            ...
            inh_raw    = (row.get("inheritance") or "").strip()
            assn       = (row.get("assnType") or "").strip().lower()
            orpha_code = (row.get("orphaCode") or "").strip()

            if not gene or not inh_raw or not orpha_code:
                continue
            if assn not in _CAUSAL_ASSN_TYPES:
                continue

            # Take first actionable inheritance mode from comma-separated list
            resolved = None
            for mode in inh_raw.split(","):
                normalised = _INH_NORMALISE.get(mode.strip().lower())
                if normalised:
                    resolved = normalised
                    break
            ...
            _GENE_DISEASE_TO_INHERITANCE[(gene, orpha_id)] = resolved
            _GENE_TO_DISEASE_INHERITANCE[gene].append((orpha_id, resolved))
            loaded += 1

    logger.info(
        f"orphanet_inheritance_tsv loaded: {loaded} causal gene-disease pairs, "
        f"{len(_GENE_TO_DISEASE_INHERITANCE)} unique genes."
    )
    _INHERITANCE_LOADED = True
...
def _load_omim_morbidmap() -> None:
    ...
    # Map OMIM inheritance keywords to short codes
    _OMIM_INH_MAP = {
        "autosomal dominant":  "AD",
        "autosomal recessive": "AR",
        "x-linked dominant":   "XLD",
    ...
    with open(path, encoding="utf-8") as fh:
        for line in fh:
            ...
            for keyword, short_code in _OMIM_INH_MAP.items():
                if keyword in phenotype:
                    _OMIM_GENE_TO_INHERITANCE[gene].add(short_code)
                    loaded += 1
                    break
...
def _get_gene_inheritance(gene: str, orpha_id: Optional[str] = None) -> Optional[str]:
    """
    Look up inheritance for a gene, preferring the specific gene-disease pair.
    
    1. If orpha_id provided → exact gene-disease pair lookup (most accurate)
    2. If no match or no orpha_id → check if all diseases for gene share same mode
       (e.g. CFTR is AR for every disease → safe to return AR)
    3. Fallback to OMIM morbidmap
    4. None if truly unknown
    """
    gene_upper = gene.upper()

    # 1. Exact gene-disease pair
    if orpha_id:
        exact = _GENE_DISEASE_TO_INHERITANCE.get((gene_upper, orpha_id))
        if exact:
            return exact

    # 2. Check if gene has only one inheritance mode across all its diseases
    disease_list = _GENE_TO_DISEASE_INHERITANCE.get(gene_upper, [])
    if disease_list:
        modes = set(inh for _, inh in disease_list)
        if len(modes) == 1:
            return next(iter(modes))  # unambiguous — all diseases same mode
        # Multiple modes and no orpha_id to disambiguate — can't pick one
        # Return most common mode as best guess
        from collections import Counter
        most_common = Counter(inh for _, inh in disease_list).most_common(1)[0][0]
        logger.info(
            f"_get_gene_inheritance: {gene} has {len(modes)} inheritance modes "
            f"across {len(disease_list)} diseases — using most common: {most_common}"
        )
        return most_common

    # 3. OMIM fallback
    omim_modes = _OMIM_GENE_TO_INHERITANCE.get(gene_upper)
    if omim_modes:
        if len(omim_modes) == 1:
            return next(iter(omim_modes))
        from collections import Counter
        return Counter(omim_modes).most_common(1)[0][0]
