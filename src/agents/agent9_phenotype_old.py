...
from src.rag.parquet_retriever import (
    query_hpo_for_gene,
    query_orphanet_inheritance,
    query_omim_gene
)
...
def _enrich_gene_phenotype_data(gene: str) -> dict:
    """
    Fetch comprehensive phenotype data from Parquet databases.

    Args:
        gene: Gene symbol (e.g., "BRCA1")

    Returns:
        Dict with keys: hpo_phenotypes, inheritance_patterns, omim_diseases
    """
    try:
        hpo_phenotypes = query_hpo_for_gene(gene, limit=100)
        inheritance = query_orphanet_inheritance(gene)
        omim_diseases = query_omim_gene(gene)
        ...
        logger.warning(f"Parquet phenotype query failed for {gene}: {e}")
        return {
            "hpo_phenotypes": [],
